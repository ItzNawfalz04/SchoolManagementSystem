...
Public Class StaffStudentRegistration
    ...
    ' Method to load all student data into StudentRegistrationDataGridView
    Private Sub LoadStudentData(Optional classID As Object = Nothing)
        Dim query As String = "SELECT s.StudentID, s.name AS StudentName, s.email AS StudentEmail, c.ClassID, c.ClassName, " &
                              ...
                              "IFNULL((SELECT SUM(su.SubjectCreditHour) * 100 FROM registrationdetail rd " &
                              "JOIN subject su ON rd.SubjectID = su.SubjectID WHERE rd.RegistrationID = r.RegistrationID), 0) AS StudentFee, " &
                              "r.RegistrationID " &
                              "FROM student s " &
        ...
        Using conn As New MySqlConnection(connectionString)
            ...
            ' Check for students without RegistrationID and assign one
            For Each row As DataRow In dataTable.Rows
                If IsDBNull(row("RegistrationID")) Then
                    Dim registrationID As Integer = AssignRegistrationID(row("StudentID"))
            ...
            With StudentRegistrationDataGridView
                ...
                .Columns("StudentFee").HeaderText = "Student Fee"
                ...
                .Columns("StudentFee").DisplayIndex = 6
    ...
    Private Sub LoadStudentSubjectData()
        ...
        Using conn As New MySqlConnection(connectionString)
            ...
            ' Set column headers
            With StudentSubjectRegistrationDataGridView
                ...
                .Columns("SubjectRemarks").HeaderText = "Remarks"
            End With
        End Using
    End Sub

    Private Sub StudentRegistrationDataGridView_CellFormatting(sender As Object, e As DataGridViewCellFormattingEventArgs) Handles StudentRegistrationDataGridView.CellFormatting
        If e.ColumnIndex = StudentRegistrationDataGridView.Columns("StudentFee").Index Then
            If e.Value IsNot Nothing AndAlso IsNumeric(e.Value) Then
                e.Value = String.Format("{0:C}", Convert.ToDecimal(e.Value)) ' Format as currency
    ...
    Private Sub StudentRegistrationDataGridView_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles StudentRegistrationDataGridView.CellClick
        If e.RowIndex >= 0 AndAlso e.RowIndex < StudentRegistrationDataGridView.Rows.Count Then
    ...
    Private Sub DisplayStudentInfo(selectedRow As DataGridViewRow)
        ...
        Dim studentFee As Decimal = totalCreditHours * 100

        TotalCreditTextBox.Text = totalCreditHours.ToString()
        StudentFeeTextBox.Text = studentFee.ToString("C") ' Format as currency
    End Sub
    ...
    Private Sub AddBtn_Click(sender As Object, e As EventArgs) Handles AddBtn.Click
        If SubjectNameComboBox.SelectedItem IsNot Nothing Then
            Dim selectedRowView As DataRowView = CType(SubjectNameComboBox.SelectedItem, DataRowView)
            Dim subjectID As Integer = Convert.ToInt32(selectedRowView("SubjectID")) ' Extract SubjectID
    ...
    Private Sub ClearBtnStudent_Click(sender As Object, e As EventArgs) Handles ClearBtnStudent.Click
        StudentRegistrationDataGridView.ClearSelection()
    ...
    Private Sub PrintDocument_PrintPage(sender As Object, e As Printing.PrintPageEventArgs)
        ...
        Dim fields As String() = {"Student ID", "Name", "Email", "Class", "Total Credit Hours", "Student Fee"}
        Dim values As String() = {StudentIDTextBox.Text, StudentNameTextBox.Text, StudentEmailTextBox.Text, ClassTextBox.Text, TotalCreditTextBox.Text, StudentFeeTextBox.Text}
        ...
        e.Graphics.DrawString("Remarks", fontSubHeader, Brushes.Black, startX + colWidth1 + colWidth2 + colWidth3, offsetY + 5)
        ...
        For Each row As DataGridViewRow In StudentSubjectRegistrationDataGridView.Rows
            If Not row.IsNewRow Then
                ...
                Dim remarks As String = row.Cells("SubjectRemarks").Value.ToString()
                ...
                e.Graphics.DrawString(remarks, fontContent, Brushes.Black, startX + colWidth1 + colWidth2 + colWidth3, offsetY + 5)
                offsetY += subjectsRowHeight
            End If
        Next
